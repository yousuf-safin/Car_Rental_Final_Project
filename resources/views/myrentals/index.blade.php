<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <h1 class="text-2xl font-bold mb-4 dark:text-white">My Rentals</h1>
                    ...
                    <ul class="divide-y divide-gray-200 dark:divide-gray-600">
                        @foreach($currentRentals as $rental)
                            <li class="py-4 flex justify-between items-center">
                                ...
                                <!-- If the booking start date is in the future, show the cancel button -->
                                @if(\Carbon\Carbon::parse($rental->start_date)->isFuture() && $rental->status != 'canceled')
                                    <form action="{{ route('rentals.cancel', parameters: $rental->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH') <!-- Change to PATCH for updating the status -->
